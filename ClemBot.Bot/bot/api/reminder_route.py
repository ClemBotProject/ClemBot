...
class ReminderRoute(BaseRoute):
    ...
    async def create_reminder(
        self, user_id: int, time: datetime, message_url: str, content: str | None, **kwargs: t.Any
    ) -> int | None:
        json = {
            "UserId": user_id,
    ...
    async def dispatch_reminder(self, reminder_id: int, **kwargs: t.Any) -> int | None:
        ...
        return t.cast(
            int | None, await self._client.patch(f"bot/reminders/{reminder_id}/dispatch", **kwargs)
        )
