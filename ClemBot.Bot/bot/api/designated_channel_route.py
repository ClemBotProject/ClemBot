...
class DesignatedChannelRoute(BaseRoute):
    ...
    async def get_guild_designated_channel_ids(self, guild_id: int, designation: str) -> list[int]:
        ...
        return t.cast(list[int], resp["mappings"])

    async def get_guild_all_designated_channels(self, guild_id: int) -> dict[str, list[int]]:
