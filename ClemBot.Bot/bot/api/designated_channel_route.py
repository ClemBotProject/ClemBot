...
class DesignatedChannelRoute(BaseRoute):
    ...
    async def register_channel(self, channel_id: int, designation: str, **kwargs):
        json = {"ChannelId": channel_id, "Designation": designation}

        return await self._client.post("bot/designatedchannels", data=json, **kwargs)

    async def delete_channel(self, channel_id: int, designation: str, **kwargs):
        json = {"ChannelId": channel_id, "Designation": designation}

        return await self._client.delete("bot/designatedchannels", data=json, **kwargs)

    async def get_guild_designated_channel_ids(self, guild_id: int, designation: str):
