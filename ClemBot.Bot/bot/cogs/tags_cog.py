...
class TagCog(commands.Cog):
    ...
    async def prefix(self, ctx, *, tag_prefix: Optional[str] = None):
        ...
        if tag_prefix in tag_prefixes:
            ...
            return
        if '`' in tag_prefix:
            await self._error_embed(ctx, "Tag prefix cannot contain the character '`'.")
            return
    ...
    @prefix.command(pass_context=True, aliases=['revert'], case_insensitive=True)
    @ext.required_claims(Claims.custom_tag_prefix_set)
    @ext.long_help(
    ...
    async def reset(self, ctx):
        if DEFAULT_TAG_PREFIX in await self.bot.get_tag_prefix(ctx.message):
            await self._error_embed(ctx, f'`{DEFAULT_TAG_PREFIX}` is already the tag prefix.')
            return
