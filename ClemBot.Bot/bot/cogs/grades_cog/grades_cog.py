...
class GradesCog(commands.Cog):
    ...
    @ext.example(
        (
            "grades math-2060",
            "grades honors math-2060",
            "grades non-honors math-2060",
            "grades all math-2060",
            "grades 2017 math-2060",
    ...
    async def grades(
        ...
        *,
        ...
        if error_title:
            ...
            embed.add_field(
                name="Help:",
                value=f"Run `{await self.bot.current_prefix(ctx)}grades list` to find what courses are available",
                inline=False,
            )
        ...
        embed.add_field(
            ...
            value=str(len(df.groupby(["Instructor"]))),
            inline=False,
        )
        embed.add_field(
            name="Want honors sections or an explanation?",
    ...
    @ext.example(
        (
            "prof brian dean",
            "prof honors kristi whitehead",
            "prof non-honors kristi whitehead",
    ...
    async def list_grades(self, ctx: ext.ClemBotCtx) -> None:
        await self.bot.messenger.publish(
            Events.on_set_pageable_embed,
    ...
    async def list_prof(self, ctx: ext.ClemBotCtx) -> None:
        await self.bot.messenger.publish(
            Events.on_set_pageable_embed,
