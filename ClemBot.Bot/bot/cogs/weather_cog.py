...
class WeatherCog(commands.Cog):
    ...
    def getPageData(self, Lat, Lon, res_weather_json, city, is_cond, is_hr, is_day):
        ...
        if is_cond:
            ...
            ix = round(float(wind_deg) / (360.0 / len(dirs)))
        ...
        for j, req_type in enumerate(req_types):
            ...
            for i, val in enumerate(forecast):
                ...
                if req_type == "day":
                    ...
                    page += (
                        f"Temperature: {round(temp, 1)}°F / {round((temp - 32) * (5 / 9), 1)}°C\n"
                    )
                ...
                page += f"Humidity: {round(hum)}%\n"
                page += (
                    f"Wind: {round(wind, 1)} mph / {round(wind * 1.609344, 1)} kmh ({wind_dir})\n\n"
                )
                pages.append(page)
    ...
    @ext.example(
        (
            ...
            "weather Clemson",
            "weather 29631",
            "weather Clemson, South Carolina",
            "weather Clemson, SC, USA",
            "weather 105 Sikes Hall, Clemson, SC 29634",
        )
    )
    async def weather(
