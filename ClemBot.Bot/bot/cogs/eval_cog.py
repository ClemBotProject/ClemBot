...
class EvalCog(commands.Cog):
    ...
    async def eval(self, ctx: ext.ClemBotCtx, *, code: str) -> None:
        ...
        log.info(
            "Code: {code} sent for evaluation by author: {author} in guild: {guild}",
            code=code,
    ...
    async def _post_eval(self, code: str) -> t.Any:
        ...
        async with aiohttp.ClientSession() as s:
            async with s.post(
                bot_secrets.secrets.repl_url, data=json_data, headers=HEADERS
            ) as resp:
                if resp.status == 200:
