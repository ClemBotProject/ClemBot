...
class TriviaCog(commands.Cog):
    ...
    @ext.short_help("Returns trivia questions")
    @ext.example("trivia")
    async def trivia(self, ctx):
        ...
        new_task = await self.asyncio_publisher(
            ctx, best_list[1]
        )  # This returns key values for our list

        # Starts the event listener for the reaction BEFORE emojis are sent

        task_1 = asyncio.create_task(
            self.send_scroll_reactions(ctx, new_task[0], new_task[1], new_task[2], new_task[4])
        )  # Sends the emojis for the reaction
        page_int = 0
        ...
        while not task_1.done():  # Loops reading the user's reaction
            ...
            if new_reaction is not None:
                page_int = await self.parse_reaction(
                    ctx,
                    new_task[0],
                    new_reaction[0],
                    new_reaction[1],
        ...
        while not task1.done():  # Loops reading the user's reaction
            ...
            if new_reaction is not None:
                page_int = await self.parse_reaction(
                    ...
                    page_int,
                    new_task[4],
                )  # client.wait_for event listeners are fine. They get unloaded/disposed of at unload
    ...
    @ext.short_help("Use this to find the category you want!")
    @commands.cooldown(1, 30, commands.BucketType.user)
    @ext.example("trivia help")
    async def list_help(
        self, ctx
    ):  # Overengineered this slightly. If the categories/difficulty/whatever else changes it will be a short fix
    ...
    def html_parser(self, new_response):
        ...
        for x in new_response["results"]:
                ...
                if isinstance(b, list):
                    ...
                    for y in b:
                        if not y.isnumeric():
                            new_list.append(
                                html.unescape(y)
                            )  # This HTML response is in a weird format where you have to navigate a list that contains dictionaries with that dictionary containing a SINGLE list
                        else:
                            new_list.append(y)
            ...
            for (
                key,
                value,
            ) in new_dictionary.items():  # .items is special because it contains an active view
        ...
        for best_loopint in range(0, dictionary_size):
            new_response["results"][best_loopint] = dictionary_list[
                best_loopint
            ]  # Sets the real dictionary to our parsed results
    ...
    async def dict_publisher(self, dictionary):
        ...
        mega_list.append(
            cog_embeds
        )  # Returns pages needed for pagination and a INDEX based value for what the answer is. e.g 0 = A 1 = B.
    ...
    async def parse_reaction(
        ...
        msg = self.messages[
            reaction.message.id
        ]  # If you actually refrence msg.curr_page_num every time it performs a lookup -> to the class rather than a constant
    ...
    async def scoreboard_embed(self, ctx, score, total_questions):
        ...
        score_embed = discord.Embed(
            title=(
                f"{current_author}'s score for this round is:  {score} out of {total_questions} this is {percent_questions}% correct"
            ),
    ...
    async def send_scroll_reactions(
        ...
        for reaction in ANSWER_KEY:
            await msg.add_reaction(
                reaction
            )  # Removed arrows because I made the decision it made the embed look too clunky, took too long to add, and added unneeded complexity
...
def helper_fixer(format_this):
    ...
    return [
        "\n".join(i) for i in chunk_sequence(new_list, CHUNK_SIZE)
    ]  # Decided to implement chunking. Although useless now if the category lists expands alot it will be an easy fix.
