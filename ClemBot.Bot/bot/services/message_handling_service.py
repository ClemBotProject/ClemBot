...
class MessageHandlingService(BaseService):
    ...
    async def on_dm_message_received(self, message: discord.Message) -> None:
        ...
        log.info(
            'Message from {author}: "{content}" Guild Unknown (DM)',
            author=serializers.log_user(message.author),
            content=message.content,
        )
    ...
    async def on_message_edit(self, before: discord.Message, after: discord.Message) -> None:
        ...
        for i, val in enumerate(before_chunk):
            embed.add_field(
                name="**Before**" if i == 0 else "Cont...", value=f"```{val}```", inline=False
            )

        for i, val in enumerate(after_chunk):
            embed.add_field(
                name="**After**" if i == 0 else "Cont...", value=f"```{val}```", inline=False
            )
        ...
        try:
            if message is not None:
                ...
                try:
                    ...
                    log.info(
                        "Uncached invalid message event received: {data}",
                ...
                embed.add_field(
                    name="Before", value="Unknown, message not stored in the database", inline=False
                )
    ...
    async def on_raw_message_delete(self, payload: discord.RawMessageDeleteEvent) -> None:
        ...
        if message is not None:
            ...
            embed.add_field(
                name="Message", value="Unknown, message not in the database", inline=False
            )
    ...
    async def handle_message_links(self, message: discord.Message) -> None:
        ...
        pattern = r"([\S\s]+)?http(s)?:\/\/(www\.)?discord(app)?\.com\/channels\/(?P<guild_id>\d{0,20})\/(?P<channel_id>\d{0,20})\/(?P<message_id>\d{0,20})\n*([\S\s]+)?"  # noqa: E501
        ...
        assert isinstance(link_channel, (discord.TextChannel, discord.Thread))
        ...
        if len(link_message.embeds) > 0:
            ...
            embed.add_field(
                name="Quoted by:", value=f"{full_name} from [Click Me]({link_message.jump_url})"
            )
